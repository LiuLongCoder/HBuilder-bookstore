			...
			<view class="contentWrapper" v-bind:key="fIdx" v-for="(floor, fIdx) in dataArray">
				...
				<view class="floorView" v-if="floor.type === 1">
					<scroll-view class="bookListScroller" scroll-x="true" scroll-y="false" show-scrollbar="false">
						<view class="bookView" v-bind:key="iIdx" v-for="(item, iIdx) in floor.items" @click="goBookDetailPage(item.itemId)">
				...
				<!-- 专区楼层 -->
				<view class="floorView" v-if="floor.type === 3 && floor.items.length > 0" @click="goSubjectBooksPage(floor.items[0].itemId, floor.items[0].moduleName)">
				...
				<view class="floorView" v-if="floor.type === 4">
					<view v-if="floor.items.length >= 2">
						...
						<view class="twoTopicView">
							<view class="oneTopicView" @click="goSubjectBooksPage(floor.items[0].itemId, floor.items[0].itemName)">
							...
							<view class="oneTopicView secondTopicView" @click="goSubjectBooksPage(floor.items[1].itemId, floor.items[1].itemName)">
				...
				<view class="floorView" v-if="floor.type === 5 && floor.items.length == 3">
					...
					<view class="thirdItemContainer">
						<view class="thirdItemCell" v-bind:key="iIdx" v-for="(item, iIdx) in floor.items" @click="goSubjectBooksPage(item.itemId, item.itemName)">
...
<script>
	...
	export default {
		...
		onShow() {
			console.log('bookstore on show')
			if (this.dataArray == null || this.dataArray.length <= 0) {
				this._request()
			}
		},
		onNavigationBarButtonTap(e) {
			console.log('>>>> tap search index', e.index)
			uni.navigateTo({
				url:'../SearchPage'
			})
		},
		methods: {
			...
			goSubjectBooksPage(recommendId, moduleName) {
				uni.navigateTo({
					url: '../SubjectBooks?recommendId=' + recommendId + '&moduleName=' + moduleName
				})
			}
		}
	}
